...
/**
 * PostgreSQL implementation of OAuthClientDb
 */
export class PostgresOAuthClientDb implements OAuthClientDb {
  private pool: Pool
  private encryptionKey: string
  private initialized = false
  ...
  getClientCredentials = async (resourceServerUrl: string): Promise<ClientCredentials | null> => {
    ...
    const result = await this.pool.query(
      'SELECT encrypted_client_id, encrypted_client_secret, redirect_uri FROM oauth_client_credentials WHERE resource_server_url = $1',
      [resourceServerUrl]
    );
  ...
  saveClientCredentials = async (
    ...
    await this.pool.query(
      'INSERT INTO oauth_client_credentials (resource_server_url, encrypted_client_id, encrypted_client_secret, redirect_uri) VALUES ($1, $2, $3, $4) ON CONFLICT (resource_server_url) DO UPDATE SET encrypted_client_id = $2, encrypted_client_secret = $3, redirect_uri = $4',
      [
        resourceServerUrl,
        encrypt(credentials.clientId, this.encryptionKey),
        encrypt(credentials.clientSecret, this.encryptionKey),
  ...
  getPKCEValues = async (state: string): Promise<PKCEValues | null> => {
    await this.ensureInitialized();
    const result = await this.pool.query(
      'SELECT encrypted_code_verifier, encrypted_code_challenge, resource_server_url FROM oauth_pkce_values WHERE state = $1',
      [state]
    );
    const row = result.rows[0];
    return row ? {
  ...
  savePKCEValues = async (
    ...
    await this.ensureInitialized();
    await this.pool.query(
      'INSERT INTO oauth_pkce_values (state, encrypted_code_verifier, encrypted_code_challenge, resource_server_url) VALUES ($1, $2, $3, $4)',
      [
        state,
        encrypt(values.codeVerifier, this.encryptionKey),
        encrypt(values.codeChallenge, this.encryptionKey),
  ...
  getAccessToken = async (resourceServerUrl: string): Promise<{
    ...
    await this.ensureInitialized();
    const result = await this.pool.query(
      'SELECT encrypted_access_token, encrypted_refresh_token, expires_at FROM oauth_access_tokens WHERE resource_server_url = $1',
      [resourceServerUrl]
    );
    const row = result.rows[0];
    if (!row) return null;
  ...
  saveAccessToken = async (
    ...
    await this.ensureInitialized();
    await this.pool.query(
      'INSERT INTO oauth_access_tokens (resource_server_url, encrypted_access_token, encrypted_refresh_token, expires_at) VALUES ($1, $2, $3, $4) ON CONFLICT (resource_server_url) DO UPDATE SET encrypted_access_token = $2, encrypted_refresh_token = $3, expires_at = $4',
      [
        resourceServerUrl,
        encrypt(token.accessToken, this.encryptionKey),
        token.refreshToken ? encrypt(token.refreshToken, this.encryptionKey) : null,
